    ...
    if (saved) {
      try {
        ...
        if (Array.isArray(parsed)) {
          ...
          // (such as restored projects) so returning visitors see them without wiping saved data.
          const missing = INITIAL_PROJECTS.filter(
            (p) => !cleaned.some((s: Project) => s.id === p.id)
          );
          if (missing.length > 0) merged = [...missing, ...cleaned];

          // Persist updated list to localStorage if items were added or deleted
          if (cleaned.length !== parsed.length || missing.length > 0) {
            try {
          ...
          // One-time thumbnail migration: swap previous stock Unsplash images for
          // real certificate preview thumbnails and SRMS dashboard screenshot. Never overwrite an image the user set manually.
          ...
          const devOpsProjectIds = [
            'proj-devops-pipeline',
            'proj-devops-iam-security',
            'proj-devops-ecr-registry',
            'proj-devops-docker-images',
            'proj-devops-docker-containers',
            'proj-devops-github-actions-history',
            'proj-devops-github-actions-steps',
            'proj-devops-ec2-server',
          ];
          merged = merged.map((p: Project) => {
            if (devOpsProjectIds.includes(p.id)) {
              const fresh = INITIAL_PROJECTS.find((f) => f.id === p.id);
              if (fresh) {
                return {
                  ...p,
